...
namespace Aide
{
    ...
    {
        static string FilePath = System.Environment.CurrentDirectory + "\\Newsini";

        #region 读Ini文件

        public static string ReadIniData(string key, string newsID)
        {
            string iniFilePath = FilePath + "\\" + newsID + ".ini";

            if (File.Exists(iniFilePath))
            {
                return INIOperationClass.INIGetStringValue(iniFilePath, "Desktop", key, null);
            }
            else
            {
                return string.Empty;
        ...
        public static bool WriteIniData(string content, string subcontent, string newsID)
        {
